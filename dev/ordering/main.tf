...
module "aws_lambda_role" {
    ...
    lambda_dynamodb_crud_policy_name = var.lambda_dynamodb_crud_policy_name
    dynamodb_table_arn = module.dynamodb_table.aws_dynamodb_table_arn
}

module "aws_lambda" {
    source                        = "../modules/aws_lambda"
    lambda_ordering_function_name = var.lambda_ordering_function_name
    lambda_exec_role_arn          = module.aws_lambda_role.lambda_exec_role_arn
    lambda_ordering_handler       = var.lambda_ordering_handler
    lambda_ordering_runtime       = var.lambda_ordering_runtime
    lambda_ordering_filename      = var.lambda_ordering_filename
    timeout                       = 20
    memory_size                   = 1024
